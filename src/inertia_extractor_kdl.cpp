...
int main(int argc, char ** argv)
{
  ...
  // Declare & get output filename parameter
  ...
  // Give other nodes (robot_state_publisher, etc.) a chance to set up if needed
  rclcpp::sleep_for(2s);

  // Get URDF from parameter 'robot_description'
  ...
  if (!model.initString(robot_description)) {
    RCLCPP_ERROR(node->get_logger(), "Failed to parse URDF from 'robot_description' string.");
    return 1;
  }
  ...
  // Map: output key -> spatial inertia 6x6
  std::map<std::string, Eigen::Matrix<double, 6, 6>> spatial_inertias;

  // Mapping from URDF link names to YAML keys
  ...
  while (current_link) {
    // Build chain from root to current_link
    KDL::Chain chain;
    if (!kdl_tree.getChain(root_name, current_link->name, chain)) {
      RCLCPP_ERROR(node->get_logger(), "Chain extraction failed for link: %s",
    ...
    // Accumulate inertia for this group
    if (current_link->inertial) {
      ...
      // Parallel axis theorem to shift to world frame about origin
    ...
    // If leaf: finalize this group and stop
    ...
    if (!child_joint || !next_link) {
      RCLCPP_ERROR(node->get_logger(),
        "Invalid child joint or link at: %s", current_link->name.c_str());
      break;
    }

    if (child_joint->type == urdf::Joint::FIXED) {
      // Still in the same “aggregated” link
      current_link = next_link;
    } else {
      // Finish the current aggregated group and start a new one
      if (current_agg.mass > 0.0 && output_key_map.count(current_group)) {
        Eigen::Vector3d com = current_agg.weighted_com / current_agg.mass;
        Eigen::Matrix3d skew = SkewSymmetric(com);
