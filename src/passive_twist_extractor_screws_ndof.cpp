...
int main(int argc, char ** argv)
{
  ...
  // ---- Read structure parameters from assembly.yaml (or assembly_6dof.yaml if you point it there) ----
  const std::string structure_file = share_dir + "/config/yaml/assembly_6dof.yaml";
  YAML::Node assembly;
  try {
  ...
  // Map: structure digit -> URDF link name of the passive joint’s moving part
  // This covers up to 3 metalinks (6 possible passive joints).
  ...
  for (const auto & param : frame_map) {
    ...
    if (!assembly[s_key]) {
      RCLCPP_WARN(
        node->get_logger(),
        "Structure digit '%s' missing in assembly.yaml; skipping '%s'.",
        s_key.c_str(), link_name.c_str());
      continue;
    }
  ...
  RCLCPP_INFO(
    node->get_logger(),
    "Active passive frames (from assembly.yaml): %zu, computed passive twists: %zu",
